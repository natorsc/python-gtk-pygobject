...
class ExampleWindow(Gtk.ApplicationWindow):
    ...
    def __init__(self, **kwargs):
        ...
        self.set_title(title='Python e GTK 4: PyGObject Gtk.FontButton()')
        ...
        self.label = Gtk.Label.new(str='Clique no botão para selecionar uma fonte.')

        vbox.append(child=self.label)

        font_button = Gtk.FontButton.new()
        font_button.set_title(title='Selecione uma fonte.')
        font_button.set_halign(Gtk.Align.CENTER)
        font_button.set_valign(Gtk.Align.CENTER)
        font_button.connect('font-set', self.on_font_button_font_set)
        vbox.append(child=font_button)
